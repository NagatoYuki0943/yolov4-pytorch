"""
为自己的数据集选择合适的yolo先验框,默认先验框来自coco数据集,本身已经很好了
利用数据集中Annotations中的xml中的object大小进行kmeans计算

参数:
    input_shape = [416, 416]    可以调整为608

运行后会输出如下,并生成kmeans_for_anchors.jpg和yolo_anchors.txt,yolo_anchors.txt直接放到model_data中即可
输出:
avg_ratio:0.67
[[ 15.808       26.23423423]
 [ 24.96        66.61921708]
 ...
 [ 99.008       94.016     ]
 [ 94.016      200.78933333]
 [235.456      168.64864865]
 [166.4        299.52      ]
 [348.608      346.43934426]]
"""
...
def load_data(path):
    ...
    for xml_file in tqdm(glob.glob('{}/*xml'.format(path))):
        ...
        for obj in tree.iter('object'):
            xmin = int(float(obj.findtext('bndbox/xmin'))) / width      # 全是小数形式
